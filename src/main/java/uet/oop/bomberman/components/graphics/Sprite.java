...
public class Sprite {
    ...
    public static final int SCALED_SIZE = DEFAULT_SIZE * SCALED_FACTOR;
    ...
    private void loadPixelFromSheet() {
        for (int y = 0; y < SPRITE_SIZE; y++) {
            for (int x = 0; x < SPRITE_SIZE; x++) {
                pixels[x + y * SPRITE_SIZE] =
                        sheet.pixels[(x + this.x) + (y + this.y) * sheet.SHEET_SIZE];
            }
        }
    }
...
//    }
    ...
    public Image getFxImage() {
        WritableImage wr = new WritableImage(SPRITE_SIZE, SPRITE_SIZE);
        PixelWriter pw = wr.getPixelWriter();
        for (int x = 0; x < SPRITE_SIZE; x++) {
            for (int y = 0; y < SPRITE_SIZE; y++) {
                if ( pixels[x + y * SPRITE_SIZE] == TRANSPARENT_COLOR) {
                    pw.setArgb(x, y, 0);
                }
                else {
                    pw.setArgb(x, y, pixels[x + y * SPRITE_SIZE]);
                }
            }
        }
    ...
    private Image resample(Image input) {
        ...
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                ...
                for (int dy = 0; dy < SCALED_FACTOR; dy++) {
                    for (int dx = 0; dx < SCALED_FACTOR; dx++) {
                        writer.setArgb(
                                x * SCALED_FACTOR + dx,
                                y * SCALED_FACTOR + dy,
                                argb
